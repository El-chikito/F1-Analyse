...
with tab_map:
    ...
    try:
        ...
        # gradient > 0 = delta croît = l'avance de D1 grandit = D1 gagne du temps ici.
        custom_scale_time = [
            [0.0, hex_to_rgb_str(c2)],   # gain négatif (gradient < 0) = D2 gagne
            [0.5, "rgb(255,255,255)"],
            [1.0, hex_to_rgb_str(c1)],   # gain positif (gradient > 0) = D1 gagne
        ]
        ...
        gain_d1 = local_gain_smooth   # D1 gagne quand gradient > 0
        gain_d2 = -local_gain_smooth  # D2 gagne quand gradient < 0
...
with tab2:
    ...
    try:
        ...
        fig.add_trace(go.Scatter(
            ...
            fillcolor=_rgba(c1, 0.30), name=f"{d1} devant", hoverinfo="skip",
        ))
        # Zone D2 devant (delta < 0)
        fig.add_trace(go.Scatter(
            x=xd, y=np.where(delta_arr < 0, delta_arr, 0.0),
            mode="lines", line=dict(width=0), fill="tozeroy",
            fillcolor=_rgba(c2, 0.30), name=f"{d2} devant", hoverinfo="skip",
        ))
        # Courbe réelle
        fig.add_trace(go.Scatter(
